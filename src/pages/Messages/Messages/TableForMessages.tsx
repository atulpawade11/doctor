...
export default function TableForMessages({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  onDelete,
  isLoading = false,
}: TableForMessagesProps) {

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Messages Available";
    const message = isFiltered
      ? "We couldn't find any messages that match your search. Try adjusting filters or search terms."
      : "There are no messages available to display at the moment.";
                ...
                {data.map((message, index) => (
                  <TableRow key={message.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    ...
                    {columns.map((col) => {
                      ...
                      let cellContent = message[col.key as keyof Message] as string;
                      
                      // Truncate quote to 30 characters (strip HTML first)
                      if (col.key === "quote") {
                        const plainText = stripHtml(cellContent);
                        cellContent = truncateText(plainText, 30);
                      }
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> messages
