...
import type { PhotoGallery } from "./photoGalleryTypes";
...
type TableForPhotoGalleryProps = {
  data: PhotoGallery[];
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: PhotoGallery) => void;
  onDelete: (item: PhotoGallery) => void;
  isLoading: boolean;
};


export default function TableForPhotoGallery({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  onDelete,
  isLoading = false,
}: TableForPhotoGalleryProps) {

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Photo Galleries Available";
    const message = isFiltered
      ? "We couldn't find any photo galleries that match your search. Try adjusting filters or search terms."
      : "There are no photo galleries available to display at the moment.";
            ...
            <Table className="w-full table-auto border-collapse">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((gallery, index) => (
                  <TableRow key={gallery.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                      ...
                      // Handle main image display
                      if (col.key === "mainImageUrl") {
                        return (
                          <TableCell key={col.key} className="px-4 py-3">
                            {gallery.mainImageUrl ? (
                              <img
                                src={gallery.mainImageUrl}
                                alt={gallery.title}
                      ...
                      // Handle multiple images count display
                      if (col.key === "multipleImages") {
                        const imageCount = gallery.multipleImages?.length || 0;
                        return (
                          <TableCell
                            key={col.key}
                      ...
                      let cellContent = gallery[col.key as keyof PhotoGallery] as string;
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> galleries
                {totalRecords > totalFiltered && (
                  <> (filtered from <strong>{totalRecords}</strong> total galleries)</>
                )}
              </>
            ) : (
              "No galleries"
