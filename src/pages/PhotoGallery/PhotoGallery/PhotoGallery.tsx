...
import TableForPhotoGallery from "./TableForPhotoGallery";
...
const generateSampleGalleries = (): PhotoGallery[] => {
  return Array.from({ length: 30 }, (_, index) => ({
    id: index + 1,
    title: `Gallery #${index + 1}`,
    // mainImageUrl: index % 4 === 0 ? `/sample-images/gallery-${index % 5}.jpg` : undefined,
...
const columnConfig = [
  { key: "mainImageUrl", label: "Main Image", visible: true, width: "0px" },
...
export default function PhotoGallery() {
  ...
  const [filteredData, setFilteredData] = useState<PhotoGallery[]>([]);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [galleryToDelete, setGalleryToDelete] = useState<PhotoGallery | null>(null);
  ...
  // Delete handlers
  const handleDeleteClick = (gallery: PhotoGallery) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    showToast("Photo gallery deleted successfully", "success");
    ...
    try {
      ...
      if (error) {
        ...
        // showToast("Failed to delete photo gallery", "error");
        // fetchGalleries(page, rowsPerPage, searchQuery);
      } else {
        setFilteredData((prev) => prev.filter((g) => g.id !== galleryToDelete.id));
        setTotalRecords((prev) => prev - 1);
      ...
      // showToast("Error deleting photo gallery", "error");
  ...
  const handleNavigateToAddEdit = (gallery?: PhotoGallery) => {
    navigate("/add-edit-photo-gallery", {
      state: {
  ...
  const handleEdit = (gallery: PhotoGallery) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <PageMeta
        title="Photo Galleries | Admin Portal"
        description="Manage photo galleries with pagination and search"
      />

      {/* Breadcrumb */}
      <div className="flex-none">
        <PageBreadCrumbForTable
          pageTitle="All Photo Galleries"
          btnLabel="Add New"
