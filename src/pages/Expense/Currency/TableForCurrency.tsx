...
export default function TableForCurrency({
  data,
  columns,
  page,
  rowsPerPage,
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 hover:scrollbar-thumb-gray-500 dark:scrollbar-thumb-gray-600 dark:hover:scrollbar-thumb-gray-500">
            <Table className="w-full table-auto border-collapse">
              <TableHeader className="sticky top-0 z-10 border-b border-gray-100 bg-white dark:bg-gray-900 dark:border-white/[0.05]">
                <TableRow>
                  {showSrNo && (
                    <TableCell isHeader className="px-3 py-3 font-medium text-gray-500 text-left text-theme-xs dark:text-gray-400 w-[60px]">
                      #
                    </TableCell>
                  )}
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((currency, index) => (
                  <TableRow key={currency.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                      ...
                      return (
                        <TableCell key={col.key} className="px-4 py-3 text-gray-600 text-theme-sm dark:text-gray-400">
                          {currency[col.key as keyof Currency] as string | number}
                        </TableCell>
                      );
                    })}
                    {showActions && (
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> currencies
