...
import TableForSalesClaims from "./TableForSalesClaims";
import CentralizedLoader from "../../../../components/common/CentralizedLoader";
import { apiService } from "../../../../services/apiService";
import { useToast } from "../../../../components/common/ToastProvider";
import type { SalesClaim, ClaimStatus, Currency } from "./salesClaimsType";
import ConfirmationModal from "../../../../components/ui/ConfirmationModal";

// Fallback sample generator for Sales Claims
const generateSampleClaims = (): SalesClaim[] => {
  const statuses: ClaimStatus[] = ["Pending", "Approved", "Rejected", "Processing"];
  const currencies: Currency[] = ["USD", "INR", "JPY"];

  const salesTeams = ["Retail", "B2B", "Online", "Wholesale"];
  const salesDesignations = ["Sales Executive", "Sales Manager", "Account Manager", "Sales Analyst"];
  const regions = ["North America", "Europe", "Asia", "Australia"];

  return Array.from({ length: 80 }, (_, i) => ({
    id: i + 1,
    claimNumber: `SCLM-${2000 + i}`, // sales claim prefix
    claimTitle: `Sales Incentive ${i + 1}`, // sales-oriented title
    employee: {
      id: i + 1,
      name: `Salesperson ${i + 1}`,
      email: `sales${i + 1}@company.com`,
      department: salesTeams[i % 4],
      designation: salesDesignations[i % 4],
      region: regions[i % 4],
      image: i % 5 === 0 ? `https://i.pravatar.cc/150?img=${(i % 70) + 1}` : undefined,
    },
    specialApproval: i % 3 === 0, // slightly different pattern
    submittedOn: new Date(Date.now() - i * 24 * 3600_000).toISOString(),
    claimAmount: 300 + i * 12.5, // sales-specific amounts
    currency: currencies[i % 3],
    passedAmountSupervisor: 280 + i * 10,
    passedAmountAuditor: 270 + i * 9,
    deductionAmount: 20 + i * 2,
    status: statuses[i % 4],
  }));
};
...
export default function SalesClaims() {
  const { showToast } = useToast();

  const [data, setData] = useState<SalesClaim[]>([]);
  ...
  const [selectedClaim, setSelectedClaim] = useState<SalesClaim | null>(null);
  ...
  const fetchClaims = useCallback(
    ...
    [page, rowsPerPage, statusFilter, specialFilter]
  );

  useEffect(() => {
    fetchClaims(page, rowsPerPage, searchQuery, statusFilter, specialFilter);
  }, [page, rowsPerPage, statusFilter, specialFilter, fetchClaims]);
  ...
  const handleDeleteClick = (claim: SalesClaim) => {
    setSelectedClaim(claim);
    setIsDeleteModalOpen(true);
  };

  const handleRestoreClick = (claim: SalesClaim) => {
    setSelectedClaim(claim);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <PageMeta title="Sales Claims | Admin Portal" description="Sales claims management" />
