...
type TableForAuditorsProps = {
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: Auditor) => void;
  onDelete: (item: Auditor) => void;
  isLoading: boolean;
};

export default function TableForAuditors({
  data,
  columns,
  page,
  rowsPerPage,
            ...
            <Table className="w-full table-auto border-collapse">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((auditor, index) => (
                  <TableRow key={auditor.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                      ...
                      switch (col.key) {
                          ...
                          return (
                            <TableCell key={col.key} className="px-4 py-3 text-left">
                              <div className="flex items-center gap-3">
                                {auditor.user.image ? (
                                  <img
                                    src={auditor.user.image}
                                    alt={auditor.user.name}
                                ...
                                {/* Fallback Icon */}
                                <div
                                  className={`w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-300 flex items-center justify-center ${auditor.user.image ? "hidden" : "flex"
                                    }`}
                                >
                                  <User size={20} />
                                </div>
                        ...
                        case "category":
                          return (
                            <TableCell key={col.key} className="px-4 py-3">
                              <span className={`inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full text-xs font-medium ${auditor.category === 'Corporate' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400'}`}>
                                {auditor.category}
                              </span>
                            </TableCell>
                          );
                        ...
                        default:
                          return (
                            <TableCell key={col.key} className="px-4 py-3 text-gray-600 text-theme-sm dark:text-gray-400">
                              {auditor[col.key as keyof Auditor] as string}
                            </TableCell>
                          );
                      }
                    })}
