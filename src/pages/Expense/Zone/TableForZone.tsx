...
import type { Zone } from "./zoneType";
...
type TableForZoneProps = {
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: Zone) => void;
  onDelete: (item: Zone) => void;
  isLoading: boolean;
};

export default function TableForZone({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  if (totalFiltered === 0) {
    ...
    const message = isFiltered
      ? "We couldn't find any zones that match your search."
      : "There's no zone data available at the moment.";
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin">
            <Table className="w-full table-auto border-collapse">
                ...
                <TableRow>
                  ...
                  {columns.filter(c => c.visible).map(col => (
                    <TableCell
                      key={col.key}
                      isHeader
                      className={`px-4 py-3 font-medium text-gray-500 text-left ${col.width ? `w-[${col.width}]` : ""}`}
                    >
                      {col.label}
                    </TableCell>
                  ))}
                  {showActions && (
                    <TableCell isHeader className="px-4 py-3 font-medium text-gray-500 text-left w-[120px]">
                      Actions
                    </TableCell>
                  )}
                </TableRow>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((zone, index) => (
                  <TableRow key={zone.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                      <TableCell className="px-3 py-3 text-left w-[60px] text-sm">
                        {firstItem + index}.
                      </TableCell>
                    )}
                    {columns.map(col => {
                      ...
                      return (
                        <TableCell key={col.key} className="px-4 py-3 text-gray-600 text-sm">
                          {zone[col.key as keyof Zone] as string}
                        </TableCell>
                      );
                    })}
                    {showActions && (
                      <TableCell className="px-4 py-3 flex gap-2 w-[120px]">
                        <button
                          onClick={() => onEdit(zone)}
                          className="rounded-full border p-2 hover:bg-gray-100 dark:hover:bg-white/[0.05]"
                        >
                          <Pencil size={16} />
                        </button>
                        <button
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> zones
