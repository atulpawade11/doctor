// TableForMeetingNotifications.tsx
import {
  Table,
  TableBody,
  TableCell,
...
import type { MeetingNotification } from "./meetingNotificationType";
...
type Props = {
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: MeetingNotification) => void;
  onDelete: (item: MeetingNotification) => void;
  isLoading: boolean;
};

export default function TableForMeetingNotifications({
  data,
  columns,
  page,
  rowsPerPage,
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 hover:scrollbar-thumb-gray-500 dark:scrollbar-thumb-gray-600 dark:hover:scrollbar-thumb-gray-500">
            <Table className="w-full table-auto border-collapse">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((item, index) => (
                  <TableRow key={item.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                      ...
                      let content: React.ReactNode = "--"; // default fallback

                      // Secretary object -> show label + EmpID
                      if (col.key === "secretary") {
                        const sec = item.secretary;
                        if (sec && sec.label) {
                          content = (
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-gray-400">{sec.EmpID || "--"}</span>
                            </div>
                          );
                        }
                      } else if (col.key === "mobileNo" || col.key === "mobileNoAlternative") {
                        const val = item[col.key as keyof MeetingNotification];
                        content = val ? String(val) : "--";
                      } else {
                        const val = item[col.key as keyof MeetingNotification];
                        content = val !== null && val !== undefined && val !== "" ? String(val) : "--";
                      }
