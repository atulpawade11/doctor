...
type TableForFloorsProps = {
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: Floor) => void;
  onDelete: (item: Floor) => void;
  isLoading: boolean;
};

export default function TableForFloors({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  if (isLoading) {
    return <CentralizedLoader isLoading={true} message="Loading floors..." />;
  }

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Floors Available";
    const message = isFiltered
      ? "We couldn't find any floors that match your search. Try adjusting filters or search terms."
      : "There are no floors available to display at the moment.";
          ...
          <Table className="w-full table-auto border-collapse">
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {data.map((floor, index) => (
                <TableRow key={floor.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                  {showSrNo && (
                  ...
                  {columns.map((col) => {
                    ...
                    return (
                      <TableCell key={col.key} className="px-4 py-3 text-gray-600 text-theme-sm dark:text-gray-400">
                        {floor[col.key as keyof Floor] as string}
                      </TableCell>
                    );
                  })}
        ...
        <div className="text-center text-gray-500 dark:text-gray-400">
          {totalFiltered > 0 ? (
            <>
              ...
              <strong>{totalFiltered}</strong> floors
              {totalRecords > totalFiltered && (
                <> (filtered from <strong>{totalRecords}</strong> total floors)</>
              )}
            </>
          ) : (
            "No floors"
