...
export default function TableForLocations({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  onDelete,
  isLoading = false,
}: TableForLocationsProps) {

  if (isLoading) {
    return <CentralizedLoader isLoading={true} message="Loading locations..." />;
  }

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Locations Available";
    const message = isFiltered
      ? "We couldn't find any locations that match your search. Try adjusting filters or search terms."
      : "There are no locations available to display at the moment.";
          ...
          <Table className="w-full table-auto border-collapse">
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {data.map((location, index) => (
                <TableRow key={location.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                  {showSrNo && (
                  ...
                  {columns.map((col) => {
                    ...
                    return (
                      <TableCell key={col.key} className="px-4 py-3 text-gray-600 text-theme-sm dark:text-gray-400">
                        {location[col.key as keyof Location] as string}
                      </TableCell>
                    );
                  })}
        ...
        <div className="text-center text-gray-500 dark:text-gray-400">
          {totalFiltered > 0 ? (
            <>
              ...
              <strong>{totalFiltered}</strong> locations
              {totalRecords > totalFiltered && (
                <> (filtered from <strong>{totalRecords}</strong> total locations)</>
              )}
            </>
          ) : (
            "No locations"
