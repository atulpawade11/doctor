...
type TableForQuoteOfTheDayProps = {
  data: QuoteOfTheDay[];
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: QuoteOfTheDay) => void;
  onDelete: (item: QuoteOfTheDay) => void;
  isLoading: boolean;
};
...
export default function TableForQuoteOfTheDay({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  onDelete,
  isLoading = false,
}: TableForQuoteOfTheDayProps) {

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Quotes Available";
    const message = isFiltered
      ? "We couldn't find any quotes that match your search. Try adjusting filters or search terms."
      : "There are no quotes available to display at the moment.";
            ...
            <Table className="w-full table-auto border-collapse">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((quote, index) => (
                  <TableRow key={quote.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                      ...
                      let cellContent = quote[col.key as keyof QuoteOfTheDay] as string;
                      
                      // Truncate description to 50 characters
                      if (col.key === "description" && cellContent.length > 50) {
                        cellContent = truncateText(cellContent, 50);
                      }
