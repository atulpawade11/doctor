...
export default function TableForEvents({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  if (totalFiltered === 0) {
    ...
    const title = isFiltered ? "No Results Found" : "No Events Available";
    const message = isFiltered
      ? "We couldn't find any events that match your search. Try adjusting filters or search terms."
      : "There are no events available to display at the moment.";
            ...
            <Table className="w-full table-auto border-collapse">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((event, index) => (
                  <TableRow key={event.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                      ...
                      let cellContent = event[col.key as keyof Event] as string;
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> events
                {totalRecords > totalFiltered && (
                  <> (filtered from <strong>{totalRecords}</strong> total events)</>
                )}
              </>
            ) : (
              "No events"
