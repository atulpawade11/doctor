...
type TableForSliderImagesProps = {
  data: SliderImages[];
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: SliderImages) => void;
  onDelete: (item: SliderImages) => void;
  isLoading: boolean;
};


export default function TableForSliderImages({
  data,
  columns,
  page,
  rowsPerPage,
                ...
                {data.map((gallery, index) => (
                  <TableRow key={gallery.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    ...
                    {columns.map((col) => {
                      ...
                      let cellContent = gallery[col.key as keyof SliderImages] as string;
