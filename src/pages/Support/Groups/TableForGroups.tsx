...
// Helper function to format members list
const formatMembers = (members: GroupMember[], maxDisplay: number = 2) => {
  if (members.length === 0) return "No members";
  if (members.length <= maxDisplay) {
    return members.map(m => m.label).join(", ");
  }
  return `${members.slice(0, maxDisplay).map(m => m.label).join(", ")} +${members.length - maxDisplay} more`;
};

export default function TableForGroups({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  if (totalFiltered === 0) {
    ...
    const title = isFiltered ? "No Results Found" : "No Groups Available";
    const message = isFiltered
      ? "We couldn't find any groups that match your search. Try adjusting filters or search terms."
      : "There are no groups available to display at the moment.";
            ...
            <Table className="w-full table-auto border-collapse">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((group, index) => (
                  <TableRow key={group.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                      ...
                      switch (col.key) {
                        case "title":
                          cellContent = group.title;
                          break;
                        case "description":
                          cellContent = truncateText(group.description, 30);
                          break;
                        // case "members":
                        //   cellContent = formatMembers(group.members);
                        //   break;
                        ...
                        case "members": {
                          ...
                          cellContent = (
                            <div className="relative group cursor-pointer">
                              ...
                              {/* Tooltip */}
                              <div className="absolute left-0 top-full mt-2 hidden group-hover:block z-20 w-60 rounded-md border border-gray-200 bg-white p-3 shadow-lg shadow-blue-200 dark:border-gray-700 dark:bg-gray-900 dark:shadow-blue-900">
                                {/* Arrow tip */}
                                <div className="absolute -top-2 left-4 w-3 h-3 rotate-45 bg-white border-l border-t border-gray-200 dark:bg-gray-900 dark:border-gray-700"></div>

                                <ul className="list-disc pl-5 space-y-1">{allMembers}</ul>
                              </div>
                            </div>
                          );
                          break;
                        ...
                        default:
                          cellContent = group[col.key as keyof Group] as string;
                      }
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> groups
                {totalRecords > totalFiltered && (
                  <> (filtered from <strong>{totalRecords}</strong> total groups)</>
                )}
              </>
            ) : (
              "No groups"
