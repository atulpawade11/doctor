...
export default function TableForCategories({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  onDelete,
  isLoading = false,
}: TableForCategoriesProps) {

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Categories Available";
    const message = isFiltered
      ? "We couldn't find any categories that match your search. Try adjusting filters or search terms."
      : "There are no categories available to display at the moment.";
            ...
            <Table className="w-full table-auto border-collapse">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((category, index) => (
                  <TableRow key={category.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                      ...
                      let cellContent = category[col.key as keyof Category] as string;
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> categories
                {totalRecords > totalFiltered && (
                  <> (filtered from <strong>{totalRecords}</strong> total categories)</>
                )}
              </>
            ) : (
              "No categories"
