...
export default function TableForLoginDetails({
  data,
  columns,
  page,
  rowsPerPage,
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03] flex flex-col">
        <div className="p-5">
          <Alert
            ...
            message="There are no login records available at the moment."
            showLink={false}
          />
        </div>
      </div>
  ...
  return (
    <>
      <CentralizedLoader isLoading={isLoading} message="Loading login details..." />
                ...
                {data.map((item, index) => (
                  <TableRow key={item.id}>
                    ...
                    {columns.map((col) => {
                      ...
                      let value = item[col.key as keyof LoginDetail] as string;
                      if (col.key === "loginTime") value = formatDateTime(value);
                      return (
