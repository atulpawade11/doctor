...
import type { PopupImage } from "./popupImagesType";
...
type TableForPopupImageProps = {
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: PopupImage) => void;
  onDelete: (item: PopupImage) => void;
  isLoading: boolean;
};

export default function TableForPopupImage({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  onDelete,
  isLoading = false,
}: TableForPopupImageProps) {

  if (isLoading) {
    return <CentralizedLoader isLoading={true} message="Loading popup images..." />;
  }

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Popup Images Available";
    const message = isFiltered
      ? "We couldn't find any popup images that match your search. Try adjusting filters or search terms."
      : "There are no popup images available to display at the moment.";
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 hover:scrollbar-thumb-gray-500 dark:scrollbar-thumb-gray-600 dark:hover:scrollbar-thumb-gray-500">
          <Table className="w-full table-auto border-collapse">
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {data.map((popupImage, index) => (
                <TableRow key={popupImage.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                  {showSrNo && (
                  ...
                  {columns.map((col) => {
                    ...
                    const cellContent = popupImage[col.key as keyof PopupImage] as string;
                    ...
                    <TableCell className="px-4 py-3 w-[120px]">
                      <div className="flex items-center justify-center gap-2 h-full">
                        ...
                        <button
                          ...
                          aria-label="Edit popup image"
                        >
                          <Pencil size={16} />
                        </button>
                        <button
                          onClick={() => onDelete(popupImage)}
        ...
        <div className="text-center text-gray-500 dark:text-gray-400">
          {totalFiltered > 0 ? (
            <>
              ...
              <strong>{totalFiltered}</strong> images
              {totalRecords > totalFiltered && (
                <> (filtered from <strong>{totalRecords}</strong> total images)</>
              )}
            </>
          ) : (
            "No images"
