...
type TableForChoreiMessageProps = {
  data: ChoreiMessage[];
  ...
  showSrNo?: boolean;
  showActions?: boolean;
  onEdit: (item: ChoreiMessage) => void;
  onDelete: (item: ChoreiMessage) => void;
  onViewPdf: (item: ChoreiMessage) => void;
  isLoading: boolean;
};

export default function TableForChoreiMessage({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  onViewPdf,
  isLoading = false,
}: TableForChoreiMessageProps) {

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Chorei Messages Available";
    const message = isFiltered
      ? "We couldn't find any chorei messages that match your search. Try adjusting filters or search terms."
      : "There are no chorei messages available to display at the moment.";
            ...
            <Table className="w-full table-auto border-collapse">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((message, index) => (
                  <TableRow key={message.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                    ...
                    {columns.map((col) => {
                     ...
                     if (col.key === "actions") {
                        ...
                        cellContent = message[col.key as keyof ChoreiMessage] as string;
                      }
                    ...
                    {showActions && (
                      <TableCell className="px-4 py-3 flex gap-2 w-[120px]">
                        <button
                          ...
                          className="flex items-center justify-center rounded-full border border-gray-300 bg-white p-2 text-sm text-gray-700 hover:bg-gray-100 transition dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.05] dark:hover:text-gray-200"
                          aria-label="Edit chorei message"
                        >
                          <Pencil size={16} />
                        </button>
                        <button
                          ...
                          aria-label="Delete chorei message"
                        >
                          <Trash2 size={16} />
                        </button>
                      </TableCell>
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> chorei messages
