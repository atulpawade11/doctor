...
export default function TableForProducts({
  data,
  columns,
  page,
  rowsPerPage,
  ...
  onDelete,
  isLoading = false,
}: TableForProductsProps) {

  if (totalFiltered === 0) {
    const isFiltered = totalRecords > 0 && totalFiltered < totalRecords;
    const title = isFiltered ? "No Results Found" : "No Products Available";
    const message = isFiltered
      ? "We couldn't find any products that match your search. Try adjusting filters or search terms."
      : "There are no products available to display at the moment.";
              ...
              <TableHeader className="sticky top-0 z-10 border-b border-gray-100 bg-white dark:bg-gray-900 dark:border-white/[0.05]">
                <TableRow>
                  {showSrNo && (
                    <TableCell
                      isHeader
                      className="px-3 py-3 font-medium text-gray-500 text-left text-theme-xs dark:text-gray-400"
                    >
                      #
                    </TableCell>
                  )}
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data.map((product, index) => (
                  <TableRow key={product.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    {showSrNo && (
                      <TableCell
                        className="px-3 py-3 text-left dark:text-white/90 text-sm"
                      >
                        {firstItem + index}.
                      </TableCell>
                    )}
                    ...
                    {columns.map((col) => {
                      ...
                      let cellContent = product[col.key as keyof Product] as string;
          ...
          <div className="text-center text-gray-500 dark:text-gray-400">
            {totalFiltered > 0 ? (
              <>
                ...
                <strong>{totalFiltered}</strong> products
                {totalRecords > totalFiltered && (
                  <> (filtered from <strong>{totalRecords}</strong> total products)</>
                )}
              </>
            ) : (
              "No products"
